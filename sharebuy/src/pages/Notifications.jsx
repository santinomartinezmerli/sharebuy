...
function Notifications() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      // 1. Posts propios (para likes, comentarios y reviews)
      ...
      // 3. Likes en mis posts (excluyo mis propios likes)
        ...
        if (likesData && likesData.length > 0) {
          ...
          likesData.forEach(l => {
            allNotifs.push({
              ...
              user_id: l.user_id,
              post_id: l.post_id,
              product: postMap[l.post_id]?.product,
      ...
      // 4. Comentarios en mis posts (excluyo mis propios comentarios)
      if (myPostIds.length > 0) {
      ...
      // 5. Reviews de posts propios (recordatorios)
      ...
      // Ordenar por fecha desc y combinar (reviews al final)
          ...
          {notifications.map(notif => {
            if (notif.type === 'follow') {
              return (
                <button
                  key={notif.id}
                  onClick={() => navigate(`/user/${notif.user_id}`)}
            ...
            if (notif.type === 'like') {
              return (
                <button
                  key={notif.id}
                  onClick={() => navigate(`/post/${notif.post_id}`)}
                  className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 text-left hover:bg-gray-50"
                >
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {notif.preview && (
                      <p className="text-xs text-gray-400 truncate mt-0.5">{notif.preview}</p>
                    )}
                  </div>
                  {notif.created_at && (
                    <span className="text-xs text-gray-400 flex-shrink-0">{timeAgo(notif.created_at)}</span>
              ...
              return (
                <div key={notif.id} className="flex items-start gap-3 px-4 py-3 border-b border-gray-100 bg-purple-50">
                  ...
                  <div className="flex-1">
                    ...
                    <button
                      onClick={() => navigate(`/review/${notif.post_id}`)}
