    ...
    <div className="flex flex-col dark:bg-gray-900 dark:text-white">
      ...
      {notifications.length === 0 ? (
        <EmptyState
          ...
          subtitle="Cuando alguien te siga o comente tus publicaciones, lo vas a ver acá"
        />
      ) : (
        <div className="flex flex-col">
          {notifications.map(notif => {
            if (notif.type === 'follow') {
              return (
                <motion.button key={notif.id} whileTap={{ scale: 0.98 }} onClick={() => navigate(`/user/${notif.user_id}`)}
                  className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-800 text-left hover:bg-gray-50 dark:hover:bg-gray-800 w-full">
            ...
            if (notif.type === 'like') {
              return (
                <motion.button key={notif.id} whileTap={{ scale: 0.98 }} onClick={() => navigate(`/post/${notif.post_id}`)}
